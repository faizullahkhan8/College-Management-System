...
const Home = () => {
    ...
    const fetchDashboardCounts = async () => {
        ...
        try {
            ...
            const [studentsRes, facultyRes, subjectsRes, branchesRes] =
            ...
            setCounts({
                students:
                    ...
                    studentsRes.value.data.success
                        ...
                        : 0,
                faculty:
                    facultyRes.status === "fulfilled" &&
                    facultyRes.value.data.success
                ...
                branches:
                    ...
                    branchesRes.value.data.success
        ...
        <div className="w-full max-w-7xl mx-auto">
            <div className="rounded-2xl p-6 md:p-8 bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg">
                ...
                <p className="mt-2 text-green-100">
                    {profileData?.designation || "Administrator"} dashboard
                    overview
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mt-6">
                {statCards.map((card) => (
                    <Link
                        key={card.title}
                        to={card.to}
                        className={`rounded-2xl p-5 shadow-sm border hover:shadow-md transition-all ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-slate-100"}`}
                    >
                        <div className="flex items-start justify-between">
                            <div>
                                ...
                                <h3
                                    className={`text-3xl font-bold mt-2 ${isDark ? "text-gray-100" : "text-slate-900"}`}
                                >
                                    {card.value}
                                </h3>
                            </div>
                        ...
                        <div
                            className={`mt-4 h-2 rounded-full overflow-hidden ${isDark ? "bg-gray-700" : "bg-slate-100"}`}
                        >
                            <div
                                className={`h-full bg-gradient-to-r rounded-full transition-all duration-500 ${isDark ? "from-green-500 to-green-600" : "from-emerald-400 to-emerald-600"}`}
                                style={{
                                    width: `${(card.value / maxCount) * 100}%`,
                                }}
                            />
                        </div>
                    </Link>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-6">
                <div
                    className={`rounded-2xl p-6 shadow-sm border ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-slate-100"}`}
                >
                    <h2
                        className={`text-lg font-semibold mb-4 ${isDark ? "text-gray-100" : "text-slate-900"}`}
                    >
                        Category Comparison
                    </h2>
                    <div className="h-80">
