...
const AddTimetableModal = ({ isOpen, onClose, onSubmit, initialData = null, branches }) => {
  ...
  const handleFileChange = (e) => {
    ...
    setFormData({ ...formData, file, previewUrl: file ? URL.createObjectURL(file) : "" });
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-8 rounded-lg w-[500px] max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold">{initialData ? "Edit Timetable" : "Add New Timetable"}</h2>
        ...
        <div className="space-y-4">
          ...
          {formData.previewUrl && (
            <div className="mt-4">
              <img src={formData.previewUrl} alt="Preview" className="max-w-full h-auto" />
            </div>
          )}

          <div className="flex justify-end gap-4 mt-6">
            <CustomButton variant="secondary" onClick={onClose}>Cancel</CustomButton>
            <CustomButton variant="primary" onClick={handleSubmit}>{initialData ? "Update" : "Add"}</CustomButton>
          </div>
        </div>
      </div>
    </div>
...
const Timetable = () => {
  ...
  const filteredTimetables = useMemo(() => {
    return timetables.filter((item) => {
      const byBranch = filters.branch ? item.branch?._id === filters.branch : true;
      const bySemester = filters.semester ? String(item.semester) === String(filters.semester) : true;
    ...
    () => [
      {
        header: "View",
        cell: ({ row }) => (
          <a href={row.original.link} target="_blank" rel="noopener noreferrer" className="text-xl">
            <MdLink />
          </a>
        ),
      },
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex justify-center gap-4">
            <CustomButton variant="secondary" onClick={() => { setEditingTimetable(row.original); setShowAddModal(true); }}>
              <MdEdit />
            </CustomButton>
            <CustomButton variant="danger" onClick={() => { setSelectedTimetableId(row.original._id); setIsDeleteConfirmOpen(true); }}>
              <MdOutlineDelete />
            </CustomButton>
          </div>
        ),
  ...
  return (
    <div className="w-full mx-auto flex justify-center items-start flex-col mb-10 relative">
      ...
      <div className="mt-8 w-full">
        <DataTable data={filteredTimetables} columns={columns} pageSize={10} />
      </div>
