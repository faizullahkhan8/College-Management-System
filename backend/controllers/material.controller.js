...
const getMaterialsController = async (req, res) => {
    try {
        ...
        if (faculty && faculty !== "undefined") query.faculty = faculty;
...
const addMaterialController = async (req, res) => {
    try {
        ...
        if (!req.file) {
            return ApiResponse.badRequest("Material file is required").send(
                res,
            );
        }
...
const updateMaterialController = async (req, res) => {
    try {
        ...
        if (type) {
            if (!["notes", "assignment", "syllabus", "other"].includes(type)) {
                return ApiResponse.badRequest("Invalid material type").send(
                    res,
                );
            }
...
const deleteMaterialController = async (req, res) => {
    try {
        ...
        return ApiResponse.success(null, "Material deleted successfully").send(
            res,
        );
    } catch (error) {
