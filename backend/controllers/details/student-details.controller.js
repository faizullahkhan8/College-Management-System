...
const updateDetailsController = async (req, res) => {
    try {
        ...
        if (phone) {
            ...
            if (existingStudent) {
                return ApiResponse.conflict("Phone number already in use").send(
                    res,
                );
            }
        }
        ...
        return ApiResponse.success(updatedUser, "Updated Successfully!").send(
            res,
        );
    } catch (error) {
...
const sendForgetPasswordEmail = async (req, res) => {
    try {
        ...
        return ApiResponse.success(null, "Reset Mail Send Successful").send(
            res,
        );
    } catch (error) {
...
const updatePasswordHandler = async (req, res) => {
    try {
        ...
        if (!resetId || !password) {
            return ApiResponse.badRequest(
                "Password and ResetId is Required",
            ).send(res);
        }
